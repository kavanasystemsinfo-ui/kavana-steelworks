...
def calcular_oee(db: Session, tenant_id) -> dict:
    ...
    # Disponibilidad: tiempo operativo real / turno estándar por línea activa
    ...
    availability = _clamp((total_tiempo / disponible) * 100) if disponible > 0 else 0.0
...
def calcular_kpis(db: Session, tenant_id) -> dict:
    ...
    cost_efficiency = (
        round((total_estimado / total_real) * 100, 1) if total_real > 0 else 0.0
    )
