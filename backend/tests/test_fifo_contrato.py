...
def test_fifo_respeta_fecha_entrada_asc(db_session, tenant, user):
    ...
    bobina_vieja = make_stock_item(
        db_session, tenant, material, cantidad=50, lote="VIEJA",
        fecha_entrada=t0 - timedelta(days=10), coste=1.5,
    )
    bobina_nueva = make_stock_item(
        db_session, tenant, material, cantidad=50, lote="NUEVA",
        fecha_entrada=t0 - timedelta(days=1), coste=2.5,
    )
...
def test_fifo_cascada_hereda_entre_bobinas(db_session, tenant, user):
    ...
    bobina_a = make_stock_item(
        db_session, tenant, material, cantidad=20, lote="A",
        fecha_entrada=t0 - timedelta(days=5), coste=1.0,
    )
    bobina_b = make_stock_item(
        db_session, tenant, material, cantidad=30, lote="B",
        fecha_entrada=t0 - timedelta(days=2), coste=3.0,
    )
    order = make_order(db_session, tenant)
    line = make_order_line(db_session, order)
